...
BOT_TOKEN = "8570594380:AAGilAAs-l8v1iWdKtGbkmkEnO0WDY6dl5I"  # ← новый токен
...
async def cb_buy(c: types.CallbackQuery):
    ...
    if c.message.chat.type in ("group", "supergroup"):
        try:
            ...
            print(f"Не удалось отправить в чат {c.message.chat.id}: {e}")
...
async def admin_start(c: types.CallbackQuery):
    ...
    for row in cur.fetchall():
        ...
        try:
            ...
            try:
                await bot.send_message(ADMIN_ID, f"Ошибка рассылки в чат {chat_id}: {e}")
            except:
                print("Не удалось уведомить админа об ошибке рассылки")
